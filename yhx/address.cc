...
namespace yhx
{
                         ...
                         int family, int type, int protocol)
    {
        ...
        if (error)
        {
            YHX_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", "
        ...
        while (next)
        {
            ...
            // YHX_LOG_INFO(g_logger) << ((sockaddr_in*)next->ai_addr)->sin_addr.s_addr;
            next = next->ai_next;
        }
                                        ...
                                        int family)
    {
        ...
        {
            YHX_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                                       " err="
        ...
        {
            YHX_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
            freeifaddrs(results);
            return false;
        }
        freeifaddrs(results);
    ...
    IPAddress::ptr IPAddress::Create(const char *address, uint16_t port)
    {
        ...
        if (error)
        {
            YHX_LOG_DEBUG(g_logger) << "IPAddress::Create(" << address
                                    << ", " << port << ") error=" << error
    ...
    IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port)
    {
        ...
        if (result <= 0)
        {
            YHX_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", "
    ...
    IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port)
    {
        ...
        if (result <= 0)
        {
            YHX_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", "
                                    << port << ") rt=" << result << " errno=" << errno
                                    << " errstr=" << strerror(errno);
            return nullptr;
        }
