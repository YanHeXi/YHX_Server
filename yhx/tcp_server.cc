...
static yhx::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout =
    yhx::Config::Lookup("tcp_server.read_timeout", (uint64_t)(60 * 1000 * 2),
                        "tcp server read timeout");

static yhx::Logger::ptr g_logger = YHX_LOG_NAME("system");

TcpServer::TcpServer(yhx::IOManager* worker, yhx::IOManager* io_worker,
                     yhx::IOManager* accept_worker)
    : m_worker(worker),
...
bool TcpServer::bind(yhx::Address::ptr addr, bool ssl) {
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs,
  ...
  for (auto& i : m_socks) {
    YHX_LOG_INFO(g_logger) << "type=" << m_type << " name=" << m_name
                           << " ssl=" << m_ssl
