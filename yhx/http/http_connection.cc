...
HttpConnection::~HttpConnection() {
  YHX_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
  ...
  if (client_parser.chunked) {
    ...
    do {
      ...
      YHX_LOG_DEBUG(g_logger) << "content_len=" << client_parser.content_len;
...
//   if (!turi) {
//     YHX_LOG_ERROR(g_logger) << "invalid uri=" << uri;
//   }
...
//                                        const std::string& vhost, uint32_t
//                                        port, bool is_https, uint32_t
//                                        max_size, uint32_t max_alive_time,
//                                        uint32_t max_request)
//     : m_host(host),
//       m_vhost(vhost),
...
//   uint64_t now_ms = yhx::GetCurrentMS();
...
//     if (!addr) {
//       YHX_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
//       return nullptr;
//     }
//     addr->setPort(m_port);
//     Socket::ptr sock =
...
//     if (!sock) {
//       YHX_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
//       return nullptr;
//     }
//     if (!sock->connect(addr)) {
//       YHX_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
//       return nullptr;
//     }
...
//   ++ptr->m_request;
//   if (!ptr->isConnected() ||
//       ((ptr->m_createTime + pool->m_maxAliveTime) >= yhx::GetCurrentMS()) ||
